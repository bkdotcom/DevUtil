 ...
 *
 * Based on \PHP_CodeSniffer\Standards\Squiz\Sniffs\Operators\ComparisonOperatorUsageSniff
 *
...
namespace bdk\DevUtil\Sniffs\Operators;
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        for ($i = $start; $i <= $end; $i++) {
            ...
            if (isset(self::$invalidOps[$tokenizer][$type]) === true) {
                $phpcsFile->addError(
                    'Operator %s prohibited; use %s instead',
                    $i,
                    $tokens[$i]['content'] . 'NotAllowed',
                    [
                        $tokens[$i]['content'],
